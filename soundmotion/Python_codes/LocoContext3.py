...
class autoSynthesis:
    ...
    def setAction(self, storagetype, data):
        if storagetype not in ['motion', 'motionbuffer', 'pose']:
        ...
        try:
            ...
            if self.fparam[storagetype]:
                ...
                if storagetype is 'motionbuffer': print "Length:", len(self.fparam['motionbuffer'])
                else: pass
                return 1
            else:
    ...
    def getFparam(self, index, sub_index=None):
        if index in ['interp','rate','gains']:
            print "Current",index,
            if index is 'interp':
    ...
    def interpolate(self, motion):
        ...
        try:
            ...
            interpolatedmotion = self.ms.interpolate(motion, self.fparam['interp']['bias'], self.fparam['interp']['tension'], self.fparam['interp']['continuity'])
    ...
    def interpolateAllMotions(self):
        ...
        self.fparam['motion'] = [self.ms.interpolate(m, self.fparam['interp']['bias'], self.fparam['interp']['tension'], self.fparam['interp']['continuity']) for m in self.fparam['motionbuffer']]
...
class locoContext:
    ...
    def updateContext(self):
        ...
        if self.keyverb is not None or self.emotion is not None or self.miscverb is not None:
            ...
            if self.emotion == 'mad' or self.emotion == 'angry':
                ...
                #set gain ++ (higher)
                self.asyn.getFparam('gains')
                
                #set interp tension ++ (higher)
                self.asyn.getFparam('interp','tension')
                ...
                self.asyn.getFparam('interp','continuity')
                self.asyn.getFparam('interp','tension')
    ...
    def executeAllMotion(self):
        self.asyn.interpolateAllMotions()
        return 1
