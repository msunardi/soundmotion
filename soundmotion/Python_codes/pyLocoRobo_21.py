...
from KHR1interface5 import khr1Interface
from dialogueModule import Dialogue
from LocoContext11 import locoContext
...
###CB1.3.2 --- Assign action to Waveshaping2 combobox
	...
	def setVariables(self):
		...
		self.context = {'user': [], 'robot': [], 'userinput':[], 'robotresponse':[]}
...
# === WAVESHAPING 2
	...
	def executeAction(self):
		if self.dialogueContext["action"] == "dance":
			...
			khr1Interface(self.khr1device, "dance").start()
		...
		if self.dialogueContext["action"] == "greet":
			khr1Interface(self.khr1device, 2).start() 
	...
	def talkToRobot(self):
		...
		#-----------------------------------------------------------
		if self.context['userinput']:
			if getUserInput == self.context['userinput'][-1]: # if user input the same sentence again...
				if self.context['userinput'].count(getUserInput) > 2:
					self.ui.textBrowserRobotResponse.append("<b><font color=blue>%s</font></b>" % ("You're repeating yourself again..."))
				else:
					self.ui.textBrowserRobotResponse.append("<b><font color=blue>%s</font></b>" % ("By the way, why are you repeating yourself?"))
		if self.context['robotresponse']:
			if response == self.context['robotresponse'][-1] and self.context['robotresponse'].count(response) > 2:
					self.ui.textBrowserRobotResponse.append("<b><font color=blue>%s</font></b>" % ("Please don't make me repeat myself again."))		
		self.context['userinput'].append(getUserInput)
		self.context['robotresponse'].append(response)
	...
	def updateDialogueContext(self, inputstring):
		...
		self.context['user'].append(self.dialogueContext['it'])
		self.context['robot'].append(self.dialogueContext['topic'])
