...
class autoSynthesis:
	...
	def setPose(self, pose):
		print "setPose()...",
		try:
			#self.fparam['pose'].append(pose)
			self.fparam['pose'] += pose
			if self.fparam['pose']:
...
class locoContext:
	...
	def __init__(self):
		...
		self.motionFilePath = {'pushup':"/home/msunardi/Documents/thesis-stuff/KHR-1-motions/push_ups_data.csv",
							   ...
							   'pose_happy':"/home/msunardi/Documents/thesis-stuff/KHR-1-motions/posture_happy.csv",
							   'pose_fear':"/home/msunardi/Documents/thesis-stuff/KHR-1-motions/posture_fear.csv",
							   'pose_sad':"/home/msunardi/Documents/thesis-stuff/KHR-1-motions/posture_sad.csv",
	...
	def lcKeywords(self, robotinput, userinput):
		print "get lcKeywords..."
		(self.miscverb, self.roboemotion, self.robokeyverb, self.topic, xname, self.roboemo) = self.lr.matchRule(robotinput)
		(self.usermiscverb, self.emotion, self.keyverb, self.usertopic, self.userwrongname, self.useremo) = self.lr.matchRule(userinput)
		
		self.userinput = userinput
		self.robotresponse = robotinput
		self.wrongname()
		...
		print "robot response:", robotinput	
		print "ROBOT: miscverb:", self.miscverb, ", emotion:", self.roboemotion, ", keyverb:", self.robokeyverb, ", topic:", self.topic
		
		print "roboemo:", self.roboemo
		print "useremo:", self.useremo
		
		return self.keyverb, self.emotion
	...
	def updateContext(self):
		...
		if self.keyverb is not None or self.emotion is not None or self.miscverb is not None:
			...
			if self.keyverb == 'wave':
				...
				concatmotion = self.asyn.concatMotion2(self.asyn.readMotion(self.motionFilePath['se_surfleft']),self.asyn.readMotion(self.motionFilePath['weirdgesture1']))
	...
	def updateContext2(self, will):	# with emotion!
		self.asyn.resetFparam()
		#self.getEmo(self.roboemo)	# << see?
		self.getEmo(self.useremo)	# << see?
		update = False
		if self.keyverb is not None or self.emotion is not None or self.miscverb is not None:
			update = True
			pose = None
			...
			if self.keyverb == 'relax':
				...
				action=self.asyn.readMotion(self.motionFilePath[actionName])
			...
			elif self.keyverb == 'surf':
				action=self.asyn.concatMotion2(self.asyn.readMotion(self.motionFilePath['se_surfleft']),self.asyn.readMotion(self.motionFilePath['se_surfright']))
				actionName='surf'
				
			elif self.keyverb == 'wave':
				actionName='wave_r_arm'
				action=self.asyn.readMotion(self.motionFilePath[actionName])
			...
			if ra < self.emoActionProbability or will==False:
			...
			print "update", update
		return update, self.keyverb, self.emotion, self.emoActionProbability
	...
	def executeAllMotion(self):
		...
		try:
			...
			if self.outputmotion == 0:
				...
				print "SUCCESS!!!"
				#self.asyn.resetFparam()
				#print "Outputmotion:", self.outputmotion			
