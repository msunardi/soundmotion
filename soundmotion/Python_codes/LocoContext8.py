...
class autoSynthesis:
    ...
    """
    def superpose(self, motion1, motion2, m2weight=0.5):
        try:
        #if True:
            tmp = []
...
class locoContext:
    ...
    def lcKeywords(self, inputstring, userinput):
        (self.miscverb, self.emotion, self.keyverb, self.topic) = self.lr.matchRule(inputstring)
        (self.usermiscverb, self.useremotion, self.userkeyverb, self.usertopic) = self.lr.matchRule(userinput)
        
        self.userinput = userinput
        self.robotresponse = inputstring
        
        print "user input:", userinput
        print "USER: miscverb:", self.usermiscverb, ", emotion:", self.useremotion, ", keyverb:", self.userkeyverb, ", topic:", self.usertopic
        
        print "robot response:", inputstring        
        print "ROBOT: miscverb:", self.miscverb, ", emotion:", self.emotion, ", keyverb:", self.keyverb, ", topic:", self.topic
    ...
    def updateContext(self):
        ...
        if self.keyverb is not None or self.emotion is not None or self.miscverb is not None:
            ...
            if self.keyverb == 'relax':
                ...
                self.updateFparam(motionbuffer=concatmotion, pose=self.asyn.readMotion(self.motionFilePath['pose_angry']))
            ...
            if self.emotion == 'mad' or self.emotion == 'angry':
                ...
                self.updateFparam(interptension=1, rate=3, pose=self.asyn.readMotion(self.motionFilePath['pose_angry']))
                ...
                self.updateFparam(pose=self.asyn.readMotion(self.motionFilePath['pose_tired']),
                                  gainslow=-2,
                                  gainsmed=-2,
                                  gainshigh=-2)
