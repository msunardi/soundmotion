...
class locoContext:
	...
	def lcKeywords(self, inputstring, userinput):
		(self.miscverb, self.robotemotion, self.keyverb, self.topic, xname) = self.lr.matchRule(inputstring)
		(self.usermiscverb, self.useremotion, self.userkeyverb, self.usertopic, self.userwrongname) = self.lr.matchRule(userinput)
		self.emotionmodifier = self.robotemotion[0]
		self.emotion = self.robotemotion[1]
		self.userinput = userinput
		self.robotresponse = inputstring
		self.wrongname()
		
		print "user input:", userinput
		print "USER: miscverb:", self.usermiscverb, "emotion modifier:", self.useremotion[0], ", emotion:", self.useremotion[1], ", keyverb:", self.userkeyverb, ", topic:", self.usertopic
		
		print "robot response:", inputstring		
		print "ROBOT: miscverb:", self.miscverb, "emotion modifier:", self.robotemotion[0], ", emotion:", self.robotemotion[1], ", keyverb:", self.keyverb, ", topic:", self.topic
	...
	def updateContext(self):
		...
		if self.keyverb is not None or self.emotion is not None or self.miscverb is not None:
			...
			if self.keyverb == 'relax':
				...
				if self.emotionmodifier is '' or self.emotionmodifier == 'slightly' and self.emotion == 'happy':
					self.updateFparam(motionbuffer=self.asyn.readMotion(self.motionFilePath['dance2']),
									  rate=3,
									  interpcontinuity=1,
									  interptension=-1,
									  gainsmed=2)
				elif self.emotionmodifier == 'very' and self.emotion == 'happy':
					self.updateFparam(motionbuffer=self.asyn.readMotion(self.motionFilePath['dance2']),
									  rate=7,
									  interpcontinuity=2,
									  interptension=-2,
			...
			if self.emotion == 'mad' or self.emotion == 'angry':
				...
				if self.emotionmodifier is '' or self.emotionmodifier == 'slightly':
					self.updateFparam(interptension=1,
									  rate=5,
									  motionbuffer=self.asyn.readMotion(self.motionFilePath['pose_angry']))
				elif self.emotionmodifier == 'very':
					self.updateFparam(interptension=2,
									  rate=5,
									  motionbuffer=self.asyn.readMotion(self.motionFilePath['weirdgesture1']))
				else:
					self.updateFparam(interptension=1,
									  rate=5,
									  motionbuffer=self.asyn.readMotion(self.motionFilePath['pose_relax']))
