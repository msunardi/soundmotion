...
class MyForm(QtGui.QMainWindow):
	...
	def loadData(self, dataIndex=None):		
		try:
			...
			self.originalData = self.ms.read(dataPath)					# read motion file
			print "loadData(): motion data loaded."
			self.ui.motionPlot.reInitialize()							# initialize plot object
			ch = 0
			for i in zip(*self.originalData):
				self.ui.motionPlot.addCurve(i) 							# plot each channel
				self.ui.motionPlot.toggleVisible(ch, self.ui.channels[str(ch)].isChecked())
				ch += 1
		except:
