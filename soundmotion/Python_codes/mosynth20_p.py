...
import KHR1readcsv2 as motion
import KHR1_motionrange2 as khr1range
import interpolation
import multiresfilter5 as mrf
import csv
import emote
import plotter
import sys
...
class MotionSynthesizer:
			...
			for index in range(len(point)-3):   #--- for each data point in the channel...
				...
				if index == len(point)-4:
					for mu in [0, 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875]:
						...
						self.p = abs(int(self.p))
						self.m_temp.append(self.p)
					self.m_temp.append(abs(int(self.interp(point[index+1], point[index+2], point[index+3], point[index+3], 1, tension, bias, continuity))))
					self.m.extend(self.m_temp)
		...
		for index in range(len(data)-3):
			...
			if index == len(data)-4:
				for mu in [0, 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875]:
					...
					self.p = float(self.p)
					self.d_temp.append(self.p)
				self.d_temp.extend(self.interp(data[index+1], data[index+2], data[index+3], data[index+3], 1, tension, bias))
				#print "d_temp type:", type(d_temp)
				self.d.extend(self.d_temp)
	...
	def adjustGain(self, channel, band, adjustment):
		...
		#print "yo!"
		return self.gains[channel][band:band+1]
	...
	#== TAKE EXTREME CAUTIONS WHEN USING THESE TWO FUNCTIONS setGains() & setDATA()
	#=== MANUALLY SET/PROVIDE GAIN VALUE (vs. from multiresFiltering()) ===
	def setGains(self, gain_data):
		print "Old gains:", self.gains,
		self.gains = gain_data
		print "New gains:", self.gains
		return 1

	#=== MANUALLY SET/PROVIDE DATA VALUE
	...
	def setDGF(self, data, gain, filtered):
		self.DATA = data
		self.gains = gain
