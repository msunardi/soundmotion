...
class Dialogue:
	...
	#self.k.respond("load aiml b")
		...
		try:
			...
			print "facedetect bypassed (dialoguemodule.py; Line 56)"
		except:
...
class FaceDetect (threading.Thread):
	def __init__(self):
		...
		self.headx = re.compile('^DISPLACEMENT_X: (\-?\d*)\s', re.IGNORECASE)
		self.heady = re.compile('^DISPLACEMENT_Y: (\-?\d*)\s', re.IGNORECASE)
		self.headhorz = re.compile('^head x: (\D*)\s')
		self.headvert = re.compile('^head y: (\D*)\s')
		...
		self.handxtolerance = 40
		self.handytolerance = 40
		self.hand_x = 0
		self.hand_y = 0
	...
	def run(self):
		...
		while True:
			if self.doRec:
				...
				headhorzmatch = self.headhorz.match(vision_output)
				headvertmatch = self.headvert.match(vision_output)
				...
				if headxmatch:
					self.head_x = int(headxmatch.group(1))
					print "Head x: ", self.head_x					
				if headhorzmatch:
					self.head_horz = headhorzmatch.group(1)
					print "(%s)" % (self.head_horz)
				#else: print ''
				if headymatch:
					self.head_y = int(headymatch.group(1))
					print "Head y: ", self.head_y
				if headvertmatch:
					self.head_vert = headvertmatch.group(1)
					print "(%s)" % (self.head_vert)
				#else: print ''
				if handxmatch:
					self.hand_x = int(handxmatch.group(1))
					print "Hand x: ", self.hand_x,
					if abs(self.hand_x) <= self.handxtolerance:
						print "(center)"
					elif self.hand_x < -self.handxtolerance:
						print "(left)"
					else:
						print "(right)"				
					
				if handymatch:
					self.hand_y = int(handymatch.group(1))
					print "Hand y: ", self.hand_y,
					if abs(self.hand_y) <= self.handytolerance:
						print "(center)"
					elif self.hand_y < -self.handytolerance:
						print "(low)"
					else:
						print "(high)"
