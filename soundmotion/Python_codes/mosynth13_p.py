...
class MotionSynthesizer:
    ...
    #=== INTERPOLATION  ===
    def interpolate( self, motion_list, bias=0, tension=-3 ):
        #self.interp = interpolation.hermiteInterpolate
        ...
        for point in motion_list:
            ...
            for index in range(len(point)-3):
                for mu in [0, 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875]:
                    self.p = self.interp(point[index], point[index+1], point[index+2], point[index+3], mu, tension, bias)
                ...
                if index == len(point)-4:
                    for mu in [0, 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875]:
                        self.p = self.interp(point[index+1], point[index+3], point[index+3], point[index+3], mu, tension, bias)
                        
                        self.p = abs(int(self.p))
                        self.m_temp.append(self.p)
                    self.m_temp.append(self.interp(point[index+1], point[index+2], point[index+3], point[index+3], 1, tension, bias))
                    self.m.extend(self.m_temp)
        ...
        for index in range(len(data)-3):
            ...
            if index == len(data)-4:
                for mu in [0, 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875]:
                    ...
                    #self.p = abs(int(self.p))
                    self.d_temp.append(self.p)
                self.d_temp.append(self.interp(data[index+1], data[index+2], data[index+3], data[index+3], 1, tension, bias))
                self.d.extend(self.d_temp)
...
# --- last changed: 2/1/09 ms = MotionSynthesizer()
