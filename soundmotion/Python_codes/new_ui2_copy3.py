...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        #self.object1 = QtGui.QTextEdit(self.tab_2)
    ...
    def addGainSliders(self, how_many):
        for i in range(how_many):
            self.gainSliders.append(QtGui.QSlider(self.tab_2))
            self.gainSliders[i].setGeometry(QtCore.QRect(20 + i*40 , 40, 16, 160))
            self.gainSliders[i].setAccessibleName(str(i))
            self.gainSliders[i].setMaximum(99)
            self.gainSliders[i].setMinimum(-99)
            self.gainSliders[i].setSliderPosition(0)
