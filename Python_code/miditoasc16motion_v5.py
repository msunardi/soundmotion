...
def processScenario(data, scenario, servo_id, beat):
	tmp = []
	l = len(data)
	for i in range(l):
		if data[i]['phrase']:		# If it's a (melodic) phrase...
							# just a reminder of the contents of phrase info: 
							# 'phrase_start', 'phrase_end', 'phrase_length', 'contour_amplitudes', 'contour_energy',
							# 'contour_time', 'notes'
			p_data = numpy.multiply(data[i]['contour_amplitudes'], data[i]['contour_energy'])
			p_data_ = [converts(int(p)) for p in p_data]
			
			print "max p_data_", max(p_data_)
			
			if data[i]['contour_energy'] != []: tmp.extend([accel(data[i]['contour_energy'][0], servo_id)])
			tmp.extend(p_data_)
		
		else:				# Otherwise it's a rest (phrase).
			tmp.extend([data[i]['rest_duration']*beat])
...
def main():
	...
	scenario=[[132,30,20,40],[54,120,54,126],[44,1,20,55,100,20]]
	nyah = processScenario(analysis,scenario,servo_id=9, beat=timeperbeat)
	...
	#molist = chan3_
	molist = formatforasc16(nyah, servo_id=1)
	#print chan3
	#for i in molist[:30]:
	#	print i
	"""
