...
def main():
	...
	chan5 = processpos2(analysis, servo_id=1, beat=timeperbeat)
	chan7 = processpos2(analysis_drum, servo_id=3, beat=timeperbeat)
	...
	chan7_ = formatforasc16(chan7, servo_id=3)
	...
	molist = combine(combine(combine(combine(chan5_,chan3_), combine(chan7_,chan9_)), combine(combine(chan11_, chan13_), combine(chan15_, chan17_))), combine(chan19_, chan21_))
	#molist = chan3_
	...
	#time.sleep(3)
	playingwithasc16.serie(molist, delay=.2, tempo=180000/len(molist))		# << uncomment to run on robot
